        ...
        {!success && (
          <form onSubmit={event => { event.preventDefault(); submit(event) }} class={data.formType}>
            <div class='mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6'>
              ...
              <div class='sm:col-span-3'>
                <label for='phone' class={`block text-sm font-regular leading-6 font-roboto ${conditionalClass}`}>Phone number</label>
                <div class='mt-2'>
                  {data.formType == 'hero' ?
            ...
            <div class='grid gap-3'>
              {/* <Checkbox v-model={data.receiveEmails}>
                <span>Yes, I want to recieve Compose email updates</span>
              </Checkbox> */}
              <button type='submit' class='bg-altus-green text-white px-16 py-4 my-8 w-full sm:w-64 rounded-full tracking-wider uppercase font-roboto font-bold text-sm'>See If I Qualify</button>
            </div>
          </form>
        )}
      </div>
